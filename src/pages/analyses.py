...
# Style partagé pour les barres de filtres
_FS = {
...
def layout() -> html.Div:
    """Retourne la page Analyses avec 7 onglets."""
    return html.Div([
        ...
        html.Div(id="analyses-content", style={"paddingTop": "12px"}),
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.RadioItems(
                    id="bar-sort",
                    options=[{"label": "Croissant", "value": "asc"},
        ...
        dcc.Graph(id="bar-graph", style={"height": "520px"}),
    ])
...
def update_barplot(variable: str, sort: str) -> go.Figure:
    """
    Barplot horizontal : médiane de la variable choisie par région.

    Parameters
    ----------
    variable : str   Colonne à agréger (rfr_moyen, taux_imposition, nb_foyers)
    sort     : str   Ordre de tri : « asc » ou « desc »
    """
    df  = load_clean_data().dropna(subset=[variable, "reg_nom"])
    agg = (df.groupby("reg_nom")[variable]
             .median()
             .reset_index(name="valeur")
             .sort_values("valeur", ascending=(sort == "asc")))

    unit = {"rfr_moyen": "€", "taux_imposition": "%", "nb_foyers": ""}[variable]
    ylab = {"rfr_moyen": "RFR moyen médian (€)",
            "taux_imposition": "Taux d'imposition médian (%)",
            "nb_foyers": "Nb foyers médian"}[variable]

    fig = go.Figure(go.Bar(
        x=agg["valeur"],
        y=agg["reg_nom"],
        orientation="h",
        marker=dict(color=agg["valeur"], colorscale="Blues",
                    showscale=True, colorbar=dict(title=ylab, thickness=12)),
        text=agg["valeur"].apply(lambda v: f"{v:,.0f}{unit}".replace(",", "\u202f")),
        textposition="outside",
        hovertemplate="<b>%{y}</b><br>" + ylab + " : %{x:,.0f}<extra></extra>",
    ))
    fig.update_layout(
        title=f"{ylab} par région",
        xaxis_title=ylab, yaxis_title="Région",
        plot_bgcolor="white", paper_bgcolor="white",
        margin={"l": 200, "r": 80, "t": 50, "b": 50}, height=520,
    )
...
def _tab_boxplot() -> html.Div:
    return html.Div([
        ...
        dcc.Graph(id="box-graph", style={"height": "600px"}),
    ])
...
def update_boxplot(variable: str, scale: str) -> go.Figure:
    """
    Boxplot horizontal par région, trié par médiane décroissante.

    Parameters
    variable : str   Colonne à afficher
    scale    : str   « linear » ou « log »
    """
    df    = load_clean_data().dropna(subset=[variable, "reg_nom"])
    order = (df.groupby("reg_nom")[variable]
               .median()
    ...
    fig.update_layout(
        ...
        xaxis_type=scale,
        plot_bgcolor="white", paper_bgcolor="white",
        margin={"l": 200, "r": 40, "t": 50, "b": 50}, height=600,
    )
...
def _tab_scatter() -> html.Div:
    ...
    return html.Div([
        ...
        html.P("Chaque point est une commune. La droite rouge est la tendance linéaire (OLS).",
        ...
        html.P(id="sc-info", style={"color": "#888", "fontSize": "0.84rem", "fontStyle": "italic"}),
    ])
...
def update_scatter(region: str, size_mode: str) -> tuple:
    """
    Scatter RFR moyen vs taux d'imposition avec droite de régression OLS.
    Echantillonne à 5 000 communes si France entière pour fluidité.

    Parameters
    region    : str   Région ou « Toutes »
    size_mode : str   « uniforme » ou « nb_foyers »
    """
    ...
    if len(df) > 5000:
        ...
        info = " Echantillon de 5 000 communes affiché pour la fluidité."
    ...
    hover = {"rfr_moyen": ":,.0f", "taux_imposition": ":.1f", "dep_nom": True}
    if size_col:
        hover["_sz"] = False
...
def _tab_topflop() -> html.Div:
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Label(id="tf-n-label",
                           children="Nb communes : 10",
...
@callback(Output("tf-n-label", "children"), Input("tf-n", "value"))
def _tf_label(n: int) -> str:
    return f"Nb communes : {n}"


@callback(
    Output("tf-graph", "figure"),
    Input("tf-region",   "value"),
    Input("tf-variable", "value"),
    Input("tf-n",        "value"),
)
def update_topflop(region: str, variable: str, n: int) -> go.Figure:
    """
    Double barplot horizontal : Top N (vert) et Flop N (rouge).

    Parameters
    
    region   : str   Région ou « Toutes »
    variable : str   Colonne de classement
    n        : int   Nombre de communes à afficher
    """
    ...
    ylab = {"rfr_moyen": "RFR moyen (€)", "taux_imposition": "Taux impo. (%)", "nb_foyers": "Nb foyers"}[variable]

    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=(f"Top {n} — {ylab}", f"Flop {n} — {ylab}"),
        horizontal_spacing=0.16,
    )
    for trace, data, color in [
        (1, top,  "#27ae60"),
        (2, flop, "#e74c3c"),
    ]:
        fig.add_trace(go.Bar(
            ...
            orientation="h",
            marker_color=color,
            text=data[variable].apply(lambda v: f"{v:,.0f}".replace(",", "\u202f")),
            textposition="outside",
            hovertemplate="<b>%{y}</b> (%{customdata})<br>" + ylab + " : %{x:,.0f}<extra></extra>",
            customdata=data["dep_nom"],
            showlegend=False,
        ), row=1, col=trace)
...
def _tab_population() -> html.Div:
    ...
    options = get_region_options(df)
    return html.Div([
        html.H3("RFR moyen selon la taille de la commune"),
...
@callback(
    ...
    Input("pop-region", "value"),
    Input("pop-type",   "value"),
)
def update_population(region: str, chart_type: str) -> go.Figure:
    """
    Barplot ou violin du RFR moyen par tranche de population.

    Parameters
    ----------
    region     : str   Région ou « Toutes »
    chart_type : str   « bar » ou « violin »
    """
    ...
    if chart_type == "bar":
        agg = (df.groupby("tranche")["rfr_moyen"]
                 ...
                 .reindex(order)
                 .reset_index())
        colors = px.colors.sequential.Blues[2: 2 + len(order)]
        fig = go.Figure(go.Bar(
            x=agg["tranche"], y=agg["median"],
            marker_color=colors,
            text=agg["median"].apply(lambda v: f"{v:,.0f} €".replace(",", "\u202f")),
            textposition="outside",
            customdata=agg["count"],
            hovertemplate="<b>%{x}</b><br>Médiane : %{y:,.0f} €<br>Nb communes : %{customdata}<extra></extra>",
        ))
        fig.update_layout(yaxis_title="RFR moyen médian (€)")
    else:
        fig = px.violin(
    ...
    fig.update_layout(
        ...
        xaxis_title="Tranche de population",
        plot_bgcolor="white", paper_bgcolor="white",
        margin={"t": 50, "b": 60, "l": 70, "r": 20}, height=480,
    )
    fig.update_yaxes(gridcolor="#f0f0f0")
    return fig
...
def _tab_pie() -> html.Div:
    return html.Div([
        html.H3("Répartition par région"),
        html.Div([
            html.Div([
                html.Label("Variable", style={"fontWeight": "600", "display": "block", "marginBottom": "4px"}),
                dcc.RadioItems(
            ...
            html.Div([
                html.Label("Style", style={"fontWeight": "600", "display": "block", "marginBottom": "4px"}),
                dcc.RadioItems(
                    id="pie-type",
...
@callback(
    ...
    Input("pie-type",     "value"),
)
def update_pie(variable: str, pie_type: str) -> go.Figure:
    """
    Pie ou donut chart de la répartition régionale.

    Parameters
    ----------
    variable : str   Colonne à agréger
    pie_type : str   « pie » ou « donut »
    """
    ...
    fig = go.Figure(go.Pie(
        ...
        marker=dict(colors=(PALETTE * 4)[:len(agg)]),
    ))
    fig.update_layout(
...
def update_heatmap(region: str) -> go.Figure:
    ...
    corr      = df[list(available.keys())].dropna().corr()
    ...
    text = [[f"{v:.2f}" if not np.isnan(v) else "" for v in row]
            for row in corr_display.values]
